...
void LogoDemo::Rotator::update(const CORBA::Any &)
{
    ...
    tx->rotate(my_zdegree, zaxis);
    my_parent->need_resize();
}

LogoDemo::LogoDemo(Application *a) : Demo(a)
{
    ...
    my_bv1 = commands->bvalue(0., 360., 0., 5., 5.);
    my_bv2 = commands->bvalue(0., 360., 0., 5., 5.);
    my_bv3 = commands->bvalue(0., 360., 0., 5., 5.);
    ...
    my_rotator1 = new Rotator(my_bv1, transformer1, fix1, -10.0);
    my_bv1->attach(Observer_var(my_rotator1->_this()));
    my_rotator2 = new Rotator(my_bv2, transformer2, fix2,  10.0);
    my_bv2->attach(Observer_var(my_rotator2->_this()));
    my_rotator3 = new Rotator(my_bv3, transformer3, fix3,  20.0);
    my_bv3->attach(Observer_var(my_rotator3->_this()));
    ...
    hbox1->append_graphic(Graphic_var(widgets->slider(my_bv1, xaxis)));
    hbox1->append_graphic(Graphic_var(layout->hfill()));
    Graphic_var hbox2 = layout->hbox();
    hbox2->append_graphic(Graphic_var(layout->hfill()));
    hbox2->append_graphic(Graphic_var(widgets->slider(my_bv2, xaxis)));
    hbox2->append_graphic(Graphic_var(layout->hfill()));
    Graphic_var hbox3 = layout->hbox();
    hbox3->append_graphic(Graphic_var(layout->hfill()));
    hbox3->append_graphic(Graphic_var(widgets->slider(my_bv3, xaxis)));
    hbox3->append_graphic(Graphic_var(layout->hfill()));
    ...
    my_application->append(bar, Babylon::String("MVC"));
}

Graphic_ptr LogoDemo::make_controller(BoundedValue_ptr value,
                      const Color &color)
{
    ToolKit_var tools = my_application->resolve<ToolKit>("IDL:fresco.org/Fresco/ToolKit:1.0");
    WidgetKit_var widgets = my_application->resolve<WidgetKit>("IDL:fresco.org/Fresco/WidgetKit:1.0");
