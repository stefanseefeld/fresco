...
namespace Prague
{
  ...
  class gzifstream : public std::istream
  {
    public:
      ...
      gzifstream(const char *name, int mode = std::ios::in) :
        std::istream(new gzbuf())
      { open(name, mode); }
      gzifstream(int fd, int mode = std::ios::in) :
        std::istream(new gzbuf())
      { attach(fd, mode); }
      virtual ~gzifstream() { delete rdbuf(); }
      gzbuf *rdbuf() { return static_cast<gzbuf *>(std::istream::rdbuf()); }
      void attach(int fd, int mode)
      {
          if (!gzifstream::rdbuf()->attach(fd, mode))
              clear(std::ios::failbit|std::ios::badbit);
          else clear();
      }
      void open(const char *name, int mode)
      {
          if (!gzifstream::rdbuf()->open(name, mode))
              clear(std::ios::failbit|std::ios::badbit);
          else clear();
      }
      void close()
      { if (!rdbuf()->close()) clear(std::ios::failbit | std::ios::badbit); }
  };
  ...
  class gzofstream : public std::ostream
  {
    ...
    public:
      ...
      gzofstream(const char *name, int mode = std::ios::out) :
        std::ostream(new gzbuf())
      { open(name, mode); }
      gzofstream(int fd, int mode = std::ios::out) :
        std::ostream(new gzbuf())
      { attach(fd, mode); }
      ...
      void attach(int fd, int mode)
      {
          if (!gzofstream::rdbuf()->attach(fd, mode))
              clear(std::ios::failbit|std::ios::badbit);
          else clear();
      }
      void open(const char *name, int mode)
      {
          if (!gzofstream::rdbuf()->open(name, mode))
              clear(std::ios::failbit|std::ios::badbit);
          else clear();
      }
      void close()
