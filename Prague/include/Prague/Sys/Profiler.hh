...
namespace Prague
{
  ...
  class Profiler
  {
    public:
      struct CheckPoint
      {
          CheckPoint(const std::string &s) :
            name(s), count(0), start(0), stop(0), elapsed(0)
          { }
          CheckPoint(CheckPoint *cp) :
            name(cp->name), count(cp->count), start(cp->start), stop(cp->stop), elapsed(cp->elapsed)
          { }
          void indent(std::ostream &os, unsigned short ind)
          { while (ind--) os.put(' '); }
