...
namespace Prague
{
  ...
  class FLock
  {
    public:
      ...
      bool trylock(off_t offset, off_t length, mode m = read)
      { return doit(offset, length, m, false) == 0; }
      //. try to activate the lock for mode @var{m}, return @var{true} if it successful}
      bool lock(mode m = read) { return doit(m, true) == 0; }
      bool lock(off_t offset, off_t length, mode m = read)
      { return doit(offset, length, m, true) == 0; }
