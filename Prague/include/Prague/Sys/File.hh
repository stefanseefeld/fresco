...
namespace Prague
{
  ...
  class File
  {
    public:
      enum type_t
      {
          none = 0, link = S_IFLNK, reg  = S_IFREG, dir  = S_IFDIR, chr  = S_IFCHR,
          blk  = S_IFBLK, fifo = S_IFIFO, sock = S_IFSOCK
      };
      enum access_t
      {
          ru = S_IRUSR, wu = S_IWUSR, xu = S_IXUSR,
          rg = S_IRGRP, wg = S_IWGRP, xg = S_IXGRP,
          ro = S_IROTH, wo = S_IWOTH, xo = S_IXOTH,
  ...
  {
      if (stat(_longname.c_str(), &_status) == -1)
      {
          _status.st_mode = 0; _error = errno; return false;
      }
      return true;
  }
