...
SDL::Console::Console(int &argc, char **argv,
    ...
    my_autoplay(false),
    my_is_gl(false),
    my_pointer_mgr(new PointerManagerT<nonGLPointer>(0)),
    my_expose(0)
{
    my_size[0] = x;
    my_size[1] = y;
    ...
    << "trying to open SDL console" << std::endl;
...
Berlin::Console::Pointer *SDL::Console::pointer(Raster_ptr raster)
{
    Prague::Trace trace("SDL::Console::pointer");
    return my_pointer_mgr->create_pointer(raster);
}

Berlin::Console::Drawable *SDL::Console::drawable()
{
    Prague::Trace trace("SDL::Console::drawable");

    if (my_drawables.empty())
    my_drawables.push_back(new SDL::Drawable(0, my_size[0],
                         my_size[1]));
                  ...
                  Fresco::PixelCoord h, Fresco::PixelCoord d)
{
    ...
    my_drawables.push_back(new SDL::Drawable("display-memory", w, h, d));
    return my_drawables.back();
}

Berlin::Console::Drawable *
SDL::Console::reference_to_servant(Fresco::Drawable_ptr drawable)
{
...
void SDL::Console::device_info(std::ostream &os)
{
    ...
    // FIXME: This should do something too.
    os << "sorry, device info isn't available for SDL at this time"
       << std::endl;
}
...
Input::Event *SDL::Console::synthesize(const SDL_Event &e)
{
    ...
    if (my_expose) my_expose->refresh_screen();
    break;
    case SDL_KEYDOWN:
    {
    Input::Toggle toggle;
    ...
    SDL_PumpEvents();
    int num = SDL_PeepEvents(move_events, 64,
                 SDL_GETEVENT, SDL_MOUSEMOTIONMASK);
    if (num > 0)
    {
        // Use last known position of mouse
        my_position[0] = move_events[num-1].motion.x;
        my_position[1] = move_events[num-1].motion.y;
    }
    else
    {
        // Use position from original event     
        my_position[0] = e.motion.x;
        my_position[1] = e.motion.y;
    }
    Input::Position position;
    position.x = my_position[0]/drawable()->resolution(Fresco::xaxis);
    position.y = my_position[1]/drawable()->resolution(Fresco::yaxis);
    position.z = 0;
    event->length(1);
    event[0].dev = 1;
...
Berlin::Console::Extension *
 SDL::Console::create_extension(const std::string & id)
{
    Prague::Trace trace("SDL::Console::create_extension()");

    if (id == "Renderer") return new SDL::Renderer();
    if (id == "GLContext")
    {
...
{
#ifdef RMDEBUG
    // I try to stay 'below' the Drawable whereever possible so that bugs
    ...
    if (!my_backup)
    my_backup = SDL_CreateRGBSurface(SDL_HWSURFACE,
...
extern "C" Berlin::Console::LoaderT<SDL::Console> *load()
{ return new Berlin::Console::LoaderT<SDL::Console>(); }
