...
namespace
{
            ...
            case Signal::segfault:
              {
                  ...
                  std::cerr << "Something went wrong. '" << output
                            << "' contains a debugging log." << std::endl
                            << "Please mail this output to bugs@fresco.org "
                            << "(together with a description" << std::endl
                            << "of your configuration and what you were doing"
                            << " before the crash)." << std::endl << std::endl;
  ...
  //. Execute a client using the command in 'value'. The process is
  //. stored in client
...
#  endif
        ...
        Logger::log(Logger::console)
          << "Console is initialized." << std::endl;
        ...
        if (!value.empty())
            props[0].value = CORBA::string_dup(value.c_str());
        ...
        Logger::log(Logger::drawing)
          << "DrawingKit is resolved." << std::endl;
        ...
        if (CORBA::is_nil(drawing))
        {
            std::cerr << "ERROR: Unable to open "
                      ...
                      << " with attribute "
                      << props[0].name << '=' << props[0].value
                      << std::endl;
            exit(3);
        }
        Logger::log(Logger::drawing)
          << "Drawing system is set up." << std::endl;
        ...
        if (use_debugger)
            debugger = new Berlin::GraphDebugger(screen->_this());
        ...
        // ---------------------------------------------------------------
        // Start the server thread - start the client listener and
        // publish it
        // ---------------------------------------------------------------
        ...
        smanager->start();
        Logger::log(Logger::loader)
          << "ScreenManager is running." << std::endl;
        ...
        Logger::log(Logger::loader)
          << "Blocking waiting for shutdown request."  << std::endl;
        sem.wait();
        Logger::log(Logger::loader)
          << "Starting server shutdown." << std::endl;
