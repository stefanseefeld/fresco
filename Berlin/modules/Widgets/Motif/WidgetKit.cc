...
namespace Berlin 
{
  namespace WidgetKit 
  {
    namespace Motif 
    {
      ...
      {
	public:
	  Forward(BoundedRange_ptr m) :
	      my_model(RefCount_var<BoundedRange>::increment(m))
	  { }
	  virtual void execute(const CORBA::Any &)
	  { my_model->forward(); }
	private:
	  RefCount_var<BoundedRange> my_model;
      };

      class Backward : public CommandImpl
      {
	public:
	  Backward(BoundedRange_ptr m) :
	      my_model(RefCount_var<BoundedRange>::increment(m))
	  { }
	  virtual void execute(const CORBA::Any &)
	      { my_model->backward(); }
...
// 	     Adapter1(typename Observer::_ptr_type o, Method m) :
//               my_ observer(o), my_method(m) {}
// 	     virtual void execute(const CORBA::Any &)
//           { (observer->*method)(); }
// 	   private:
// 	     typename Observer::_ptr_type my_observer;
// 	     Method my_method;
//       };
	  ...
	  my_strut = my_layout->fixed_size(Fresco::Graphic::_nil(),
					   80., 80.);
	  ...
	  Graphic_var outset = my_tools->frame(g, 20, s2, true);
	  Graphic_var frame = 
	      my_tools->create_switch(inset, outset,
				      Fresco::Controller::toggled, toggle);
	  toggle->body(frame);
	  return toggle._retn();
      }
	  ...
	  // FIXME: What happens if the axis == zaxis? Throw an exception
	  //        since it is a stupid idea to request it? -- tobias
	  Slider *slider = new Slider(value, axis, req);
	  activate(slider);

	  // the thumb
	  Graphic_var box = axis == xaxis ? my_layout->hbox() :
	      my_layout->vbox();
	  spec.brightness(0.5); spec._d(Fresco::ToolKit::outset);
	  ...
	  Controller_var thumb =
	      my_tools->dragger(box,
				Command_var(slider->create_adjust_cmd()));
	  slider->init(thumb);

	  // now put it into an inset
	  spec.brightness(0.5); spec._d(Fresco::ToolKit::inset);
	  Graphic_var inset =
	      my_tools->frame(Graphic_var(slider->_this()), 20.,
			      spec, false);
	  Controller_var root =
      ...
      Controller_ptr WidgetKit::panner(BoundedRange_ptr x,
				       BoundedRange_ptr y)
      {
	  ...
	  Fresco::ToolKit::FrameSpec spec;
	  spec.brightness(0.5); spec._d(Fresco::ToolKit::outset);
	  Graphic_var outset = my_tools->frame(Fresco::Graphic::_nil(),
					       20., spec, true);
	  Controller_var thumb =
	      my_tools->dragger(outset,
				Command_var(panner->create_adjust_cmd()));
	  panner->init(thumb);
	  
	  spec.brightness(0.5); spec._d(Fresco::ToolKit::inset);
	  Graphic_var fixed =
	      my_layout->fixed_size(Graphic_var(panner->_this()),
				    1000., 1000.);
	  ...
	  Controller_var lower =
	      my_tools->stepper(Graphic::_nil(),
				Command_var(backward->_this()));
	  
	  CommandImpl *forward = new Forward(x);
	  activate(forward);
	  Controller_var upper =
	      my_tools->stepper(Graphic::_nil(),
				Command_var(forward->_this()));
	  ...
	  Graphic_var box = a == xaxis ? my_layout->hbox() :
	      my_layout->vbox();
...
//       Widget::Splitter_ptr WidgetKit::splitter(Graphic_ptr left,
//                                                Graphic_ptr right,
//                                                Axis a)
//       {
//           BoundedValue_var adjustment =
//               my_commands->bvalue(0., 1., 0., 0.01, 0.1);
...
extern "C" Berlin::KitImpl *load()
{
    ...
    return Berlin::create_prototype<Berlin::WidgetKit::Motif::WidgetKit> ("IDL:fresco.org/Fresco/WidgetKit:1.0", properties, 4);
}
