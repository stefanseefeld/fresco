...
public:
  Activate::Activate(GLuint *tx, Fresco::PixelCoord *width,
    ...
    : my_tx(tx), my_width(width), my_height(height) {}
  void operator()() {
    ...
    GLfloat tPlane[4] = { 0.0, -drawable->resolution(Fresco::yaxis)/(*my_height), 0.0, 0.0 };
...
public:
  Bind::Bind(GLint c, GLenum f, unsigned char *d, Texture *t)
    : components(c), format(f), data(d), that(t) {}
  void operator()() {
    GLuint texture;
    ...
    logbase2(that->width, w);
    if (w > maxsize) w = maxsize;
    logbase2(that->height, h);
    ...
    if (w != that->width || h != that->height)
      {
        ...
        scale_image(format, that->width, that->height, data, w, h, image);
      }
    else image = data;
...
{
public:
  Delete::Delete(GLuint *tx)
    : my_tx(tx) {}
  void operator()() {
    glDeleteTextures(1, my_tx);
  }
private:
  GLuint *my_tx;
};
...
openGL::Texture::Texture(Fresco::Raster_var r)
  : remote(Fresco::Raster::_duplicate(r)), texture(0), is_bound(false)
{
...
{
  if (is_bound) my_glcontext->add_to_queue(new Delete(&texture));
}
...
void openGL::Texture::activate(GLContext *glcontext)
{
  ...
  glcontext->add_to_queue(new Activate(&texture, &texgen_w, &texgen_h));
}
...
public:
  Activate::Activate(GLuint *tx, Fresco::PixelCoord *width,
    ...
    : my_tx(tx), my_width(width), my_height(height), my_lesser_height(h2) {}
  void operator()() {
    ::Console::Drawable *drawable = ::Console::instance()->drawable();
...
public:
  Bind::Bind(GLint c, GLenum f, unsigned char *d, Image *i)
    : components(c), format(f), data(d), that(i) {}
  void operator()() {
    GLuint texture;
    glGenTextures(1, &texture);
    glBindTexture(GL_TEXTURE_2D, texture);
    ...
    if (w != that->width || h != that->height)
      {
        ...
        image = new unsigned char [(w+4) * h * bpp];
        resize_image(format, that->width, that->height, data, w, h, image);
        that->s = static_cast<GLfloat>(that->width)/w;
        that->t = static_cast<GLfloat>(that->height)/h;
      }
    else image = data;
