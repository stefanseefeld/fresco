 ...
 * MA 02139, USA.
 */
#include "Drawing/openGL/GLPolygonizer.hh"
#include <GL/gl.h>
#include <GL/glu.h>

using namespace FT;

GLPolygonizer::Handler  GLPolygonizer::Handler::defaultHandler(false);
GLPolygonizer::Handler *GLPolygonizer::Handler::handler;

bool GLPolygonizer::polygonize(double precision, Handler *handler)
{
  ...
  cout << "Polygonizer::polygonize" << endl;
...
void GLPolygonizer::Handler::begin(int type)
{
  glBegin(GLenum(type));
}

void GLPolygonizer::Handler::vertex(FT::Outline::Point *point)
{
  if(!point) return;
  glVertex2f(point->x, point->y);
}

void GLPolygonizer::Handler::end()
{
  glEnd();
}

void GLPolygonizer::Handler::error(int error)
{
  if(!verbose) return;
...
void GLPolygonizer::Handler::handleBegin(GLenum type)
{
  if (handler) handler->begin(int(type));
}

void GLPolygonizer::Handler::handleVertex(void *data)
{
  if (handler) handler->vertex(reinterpret_cast<Outline::Point *>(data));
}

void GLPolygonizer::Handler::handleEnd()
{
  if (handler) handler->end();
}

void GLPolygonizer::Handler::handleError(GLenum error)
{
