...
void openGL::Quadric::cylinder(double baseRadius, double topRadius,
			       double height, int slices, int stacks)
{
    double nsign = my_orient == in ? -1. : 1.;
    double da = 2.0*M_PI / slices;
    double dr = (topRadius - baseRadius) / stacks;
    double dz = height / stacks;
    // Z component of normal vectors
    double nz = (baseRadius - topRadius) / height;

//    if (my_style == GLU_POINT)
//    {
//        glBegin(GL_POINTS);
...
//        glEnd();
//    }
    if (my_style == Fresco::DrawingKit::outlined)
...
//       else
	...
	for (int j = 0; j < stacks; j++)
	{
		...
		{
		    normal3d(x*nsign, y*nsign, nz*nsign);
		    if (my_style == Fresco::DrawingKit::textured)
			glTexCoord2f(s, t);
		    glVertex3d(x * r, y * r, z);
		    normal3d(x*nsign, y*nsign, nz*nsign);
		    if (my_style == Fresco::DrawingKit::textured)
...
void openGL::Quadric::sphere(double radius, int slices, int stacks)
{
    ...
    // texturing: s goes from 0.0/0.25/0.5/0.75/1.0 at
    // +y/+x/-y/-x/+y axis
    // t goes from -1.0/+1.0 at z = -radius/+radius (linear along
	...
	if (my_style != Fresco::DrawingKit::textured)
	{
		...
		if (my_norm)  glNormal3d(x*nsign, y*nsign, z*nsign);
	...
	int imin = my_style == Fresco::DrawingKit::textured ? 0 : 1;
	int imax = my_style == Fresco::DrawingKit::textured ? stacks :
	    stacks - 1;
    ...
    else if (my_style == Fresco::DrawingKit::outlined)
    {
	// draw stack lines
...
void openGL::Quadric::disk(double innerRadius, double outerRadius,
			   int slices, int loops)
{
    // Normal vectors
    if (my_norm != none)
    {
	if (my_orient == out) glNormal3d(0.0, 0.0, +1.0);
	...
	for (int l = 0; l < loops; l++)
	{
		...
		for (int s = slices; s >= 0; s--)
		{
		    ...
		    double sa = sin(a), ca = cos(a);
		    if (my_style == Fresco::DrawingKit::textured)
			glTexCoord2f(0.5 - sa * r2 / dtc,
				     0.5 + ca * r2 / dtc);
		    glVertex2f( r2*sa, r2*ca);
		    if (my_style == Fresco::DrawingKit::textured)
			glTexCoord2f(0.5 - sa * r1 / dtc,
				     0.5 + ca * r1 / dtc);
		    glVertex2f( r1*sa, r1*ca);
		}
		glEnd();
...
void openGL::Quadric::partialDisk(double innerRadius, double outerRadius,
				  int slices, int loops,
...
//     }
	...
	if (innerRadius > 0.0)
	{
	    ...
	    for (int slice = 0; slice < slices; slice++)
	    {
		glVertex2d(innerRadius * sin(angle),
