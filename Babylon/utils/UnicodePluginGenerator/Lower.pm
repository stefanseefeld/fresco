...
sub var_def {
  ...
  if ($self->{_ATTENTION_NEEDED}) {
    return "    static const _UCS4 lower\[$bl_length\];\n";
  } else {
    return "";
  }
}
...
sub var {
  ...
  if ($self->{_ATTENTION_NEEDED}) {
    my $tmp = "  const _UCS4 $bl_name\:\:lower\[\] = {";
    for (my $i= $bl_start; $i <= $bl_end; $i++) {
      if (($i - $bl_start) % 8 == 0) {
	$tmp .= "\n    ";
      }
