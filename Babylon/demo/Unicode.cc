...
int main (int argc, char **argv) {
  ...
  for ( ; A < _UCS4(0xFFFFFFFF) || runOverZero; A++) {
    runOverZero = 0;
    ...
    if (A > 0xF0000 && A < 0xFFFFD) {
	chars++; continue;
    }
    if (A > 0x100000 && A < 0x10FFFD) {
	chars++; continue;
    }

    if(!A.is_defined()) { continue; } // breaks the loop for undefined chars

    try {
      chars++;
      
      // code value
      cout << hex << setfill('0') << setw(8) << A.myUnicode() << ";" << flush;
      // Gen_Cat
      switch(A.category()) {
    ...
    catch (Babylon::FileError &Except) {
      cerr << "ERROR: "
	   << Except.ErrorFilename << ": "
	   << Except.ErrorMessage << "\n" << flush;
    } // catch
    catch (Babylon::BlockError &Except) {
      cerr << "ERROR: Blockerror" << endl << flush;
    } // catch
    catch (Babylon::UndefinedProperty &Except) {
      cerr << "ERROR: UndefinedProperty" << endl << flush;
    }
  } // for
