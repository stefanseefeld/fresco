 ...
 * ftp.unicode.org on Tue, 21 Nov 2000 21:12:50 +0100.
 *
...
namespace Babylon {
  ...
  public:
    ...
    Line_Break linebreak(const UCS4 & _uc) const {
      if (!isDefined(_uc))
        throw UndefinedProperty(_uc, PROP_CHARACTER);
      return Babylon::Line_Break(Box_Drawing2500::lb[_uc - my_first_letter]);
    }

    EA_Width EAWidth(const UCS4 & _uc) const {
      if (!isDefined(_uc))
        throw UndefinedProperty(_uc, PROP_CHARACTER);
      return Babylon::EA_Width(Box_Drawing2500::ea[_uc - my_first_letter]);
    }
  ...
  const unsigned char Box_Drawing2500::lb[] = {
  ...
  const unsigned char Box_Drawing2500::ea[] = {
