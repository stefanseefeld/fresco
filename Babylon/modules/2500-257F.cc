 ...
 * ftp.unicode.org on Wed,  6 Dec 2000 23:25:48 +0100.
 *
...
namespace Babylon {
  ...
  public:
    ...
    Line_Break linebreak(const UCS4 uc) const {
      if (!is_defined(uc))
        return LB_MAX;
      return Babylon::Line_Break(Box_Drawing2500::_lb[uc - _first_letter]);
    }

    EA_Width EA_width(const UCS4 uc) const {
      if (!is_defined(uc))
        return EA_WIDTH_MAX;
      return Babylon::EA_Width(Box_Drawing2500::_ea[uc - _first_letter]);
    }
  ...
  const unsigned char Box_Drawing2500::_lb[] = {
  ...
  const unsigned char Box_Drawing2500::_ea[] = {
