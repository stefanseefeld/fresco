 ...
 * ftp.unicode.org on Wed, 15 Nov 2000 23:27:16 +0100.
 *
...
namespace Babylon {
  ...
  class Box_Drawing : public Babylon::Dictionary::Block {
  public:
    void clean () {
    };
    ...
    Line_Break linebreak(const _UCS4 _uc) const {
      if (!isDefined(_uc))
        throw UndefinedProperty(_uc, PROP_CHARACTER);
      return Babylon::Line_Break(Box_Drawing::lb[_uc - my_first_letter]);
    }

    EA_Width EAWidth(const _UCS4 _uc) const {
      if (!isDefined(_uc))
        throw UndefinedProperty(_uc, PROP_CHARACTER);
      return Babylon::EA_Width(Box_Drawing::ea[_uc - my_first_letter]);
    }
  ...
  const unsigned char Box_Drawing::lb[] = {
  ...
  const unsigned char Box_Drawing::ea[] = {
