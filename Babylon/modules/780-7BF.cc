/*$Id: UnicodePluginGenerator.pl,v 1.6 2002/05/31 23:42:14 tobias Exp 780-7BF.cc
 *
...
namespace Babylon
{
  namespace Module
  {
    class Thaana780 : public Babylon::Block
    {
      public:
        void clean() { };
	...
	std::string blockname(const UCS4 uc) const
	{
        ...
        Gen_Cat category(const UCS4 uc) const
        {
            if (!is_defined(uc))
                return CAT_MAX;
            return Babylon::Gen_Cat(Thaana780::my_cat[uc - my_first_letter]);
        }
        ...
        Bidir_Props bidir_props(const UCS4 uc) const
        {
            ...
            return Thaana780::my_bidir[uc - my_first_letter];
        }
        ...
        Line_Break linebreak(const UCS4 uc) const
        {
            if (!is_defined(uc))
                return LB_MAX;
            return Babylon::Line_Break(Thaana780::my_lb[uc - my_first_letter]);
        }
      ...
      private:
        // functions
        Thaana780(const Thaana780 &) ; // no implementaion!
    ...
    const std::bitset<64> Thaana780::my_is_defined(std::string("0000000000000011111111111111111111111111111111111111111111111111"));

    const unsigned char Thaana780::my_cat[] =
    {
    ...
    const Babylon::Bidir_Props Thaana780::my_bidir[] =
    {
    ...
    const unsigned char Thaana780::my_lb[] =
    {
    ...
    const std::bitset<64> Thaana780::my_Other_Alphabetic(std::string("0000000000000001111111111100000000000000000000000000000000000000"));

    const std::bitset<64> Thaana780::my_Diacritic(std::string("0000000000000001111111111100000000000000000000000000000000000000"));

  }; // namespace
}; // namespace

dload(Babylon::Module::Thaana780);
